...
public interface CoursesRepository extends JpaRepository<Courses, Integer> {
	...
	@Query(value="select concat(t.teacher_fname,\" \",t.teacher_lname) from teacher t,courses c where t.teacher_id=c.course_teacher_id and course_id=:course_id;",nativeQuery=true)
	public String getTeacherByCourseId(@PathVariable("course_id") int course_id);
	
	
	
	@Query(value="select course_teacher_id from courses where course_id= ?1",nativeQuery=true)
	public int getTeacherId(@PathVariable("course_id") int course_id);
